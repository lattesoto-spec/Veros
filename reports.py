...
def compute_quarter_stats(facility_id: int, year: int, quarter: int) -> dict:
    ...
    for s, st in shifts:
        ...
        if role in worked_minutes:
            worked_minutes[role] += shift_worked_minutes(s)
...
def build_quarterly_pdf(facility, year: int, quarter: int) -> bytes:
    ...
    story.append(Paragraph(
        "Draft generated by CareMin for internal review. Figures derive from rostered direct-care "
        "shifts and resident occupancy in the prototype dataset. Labour costs are not tracked in "
        "CareMin and must be entered from payroll before submission. Independent audit under "
    ...
    story.append(Paragraph(
        "CareMin does not store payroll costs. Enter quarterly direct-care salary + superannuation "
